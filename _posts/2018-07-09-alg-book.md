...
2. Пока left <= right:
    ...
    2.3. В противном случае (A[q] != x), если A[q] > x, установить left = q - 1
    2.4. В противном случае (A[q] < x), установить r = q + 1
...
<details>
    ...
    <pre><code class="perl">my @array = (0..99);
my $length = scalar(@array);
my $item = 9;
my $index = binary_search(\@array, $length, $item);

print $index eq 'not found' ? "$index\n" : "Item = $item; index = $index; check = $array[$index]\n";
...
sub binary_search {
    ...
    while ( $left <= $right ){
        ...
        $array->[$middle] > $item ? $left-- : $right++;
    }
    return 'not found';
}

$ perl script.pl 
Item = 9; index = 9; check = 9
...
timethese(100, {
    'binary' => sub {
        my $item = int rand @arr;
        my $index = binary_search(\@arr, $length, $item);
    }
});

$ perl script.pl 
Benchmark: timing 100 iterations of binary...
    binary: 22 wallclock secs (20.59 usr +  0.01 sys = 20.60 CPU) @  4.85/s (n=100)
    </code></pre>
Просто ужасная скорость... 20 секунд. 
</details>
